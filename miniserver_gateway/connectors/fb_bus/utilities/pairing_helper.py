...
class PairingHelper:
    ...
    def receive(
        ...
        if packet == Packets.FB_PACKET_PAIR_DEVICE:
            ...
            if pairing_response == PairingResponses.FB_PAIRING_RESPONSE_DEVICE_ADDRESS:
                ...
                self.__device_address_receiver(sender_address, payload)

            elif (
                pairing_response
                == PairingResponses.FB_PAIRING_RESPONSE_ADDRESS_ACCEPTED
            ):
                log.debug("Received pairing response: Device confirmed address")

                self.__device_address_accepted_receiver(sender_address, payload)
                ...
                self.__device_about_info_receiver(sender_address, payload)

            elif (
                pairing_response == PairingResponses.FB_PAIRING_RESPONSE_DEVICE_MODEL
                or pairing_response
                == PairingResponses.FB_PAIRING_RESPONSE_DEVICE_MANUFACTURER
                or pairing_response
                == PairingResponses.FB_PAIRING_RESPONSE_DEVICE_VERSION
                or pairing_response
                == PairingResponses.FB_PAIRING_RESPONSE_FIRMWARE_MANUFACTURER
                or pairing_response
                == PairingResponses.FB_PAIRING_RESPONSE_FIRMWARE_VERSION
            ):
                log.debug("Received pairing response: Device description")

                self.__device_description_receiver(
                    pairing_response, sender_address, payload
                )

            elif (
                pairing_response == PairingResponses.FB_PAIRING_RESPONSE_REGISTERS_SIZE
            ):
                log.debug("Received pairing response: Device registers sizes")

                self.__registers_size_receiver(sender_address, payload)

            elif (
                pairing_response
                == PairingResponses.FB_PAIRING_RESPONSE_REGISTERS_STRUCTURE
            ):
                log.debug("Received pairing response: Device registers structure")

                self.__registers_structure_receiver(sender_address, payload, length)
                ...
                self.__settings_size_receiver(sender_address, payload)

            elif (
                pairing_response
                == PairingResponses.FB_PAIRING_RESPONSE_SETTINGS_STRUCTURE
            ):
                log.debug("Received pairing response: Device settings structure")

                self.__settings_structure_receiver(sender_address, payload, length)

            elif pairing_response == PairingResponses.FB_PAIRING_RESPONSE_FINISHED:
                log.debug("Received pairing response: Pairing finished")

                self.__pairing_finished_receiver(sender_address, payload)
    ...
    def handle(self) -> None:
        ...
        if self.__pairing_device is None:
            ...
            # Search timeout is not reached, new devices could be searched
                ...
                or time.time() - self.__last_pairing_request_broadcast
                >= self.__SEARCHING_DELAY
            ):
        ...
        # Device for pairing is assigned
            ...
            pairing_device: DeviceEntity = self.__connector.get_device_by_id(
                self.__pairing_device
            )
            ...
            if pairing_device.get_attempts() >= self.__MAX_TRANSMIT_ATTEMPTS:
                log.debug(
                    "Pairing for device with address: {} could not be finished. Device is lost".format(
                        pairing_device.get_address()
                    )
                )
            ...
            if (
                ...
                == PairingCommands.FB_PAIRING_CMD_SET_ADDRESS
            ):
                if pairing_device.get_state() != DeviceStates.STATE_CONNECTED:
                    log.debug(
                        "Device is in invalid state, paring could not be finished".format(
                            pairing_device.get_address()
                        )
                    )
                ...
                if (
                    pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_ABOUT_INFO
                    or pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_DEVICE_MODEL
                    or pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_DEVICE_MANUFACTURER
                    or pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_DEVICE_VERSION
                    or pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_FIRMWARE_MANUFACTURER
                    or pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_FIRMWARE_VERSION
                    or pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_REGISTERS_SIZE
                    or pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_SETTINGS_SIZE
                    or pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_FINISHED
                ):
                    self.__send_pairing_cmd_handler(
                        pairing_device, pairing_device.get_pairing_cmd()
                    )

                elif (
                    pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_REGISTERS_STRUCTURE
                ):
                    self.__send_provide_registers_structure_handler(pairing_device)

                elif (
                    pairing_device.get_pairing_cmd()
                    == PairingCommands.FB_PAIRING_CMD_PROVIDE_SETTINGS_STRUCTURE
                ):
    ...
    def __configure_registers(
        self, device: DeviceEntity, registers_size: int, registers_type: RegistersTypes
    ) -> None:
        ...
        for i in range(registers_size):
            ...
            if register is not None:
                ...
                self.__connector.create_register(
                    ...
                    registers_type,
                    DataTypes(
                        DataTypes.FB_DATA_TYPE_BOOL
                        if is_binary is True
                        else DataTypes.FB_DATA_TYPE_UNKNOWN
                    ),
                )

        if registers_size < len(
            self.__connector.get_registers_by_type(device, registers_type)
        ):
            for i in range(
                registers_size,
                len(self.__connector.get_registers_by_type(device, registers_type)),
            ):
                register: RegisterEntity or None = (
                    self.__connector.get_register_by_address(device, registers_type, i)
                )
    ...
    def __configure_settings(
        self, device: DeviceEntity, settings_size: int, settings_type: SettingsTypes
    ) -> None:
        for i in range(settings_size):
            setting: DeviceSettingEntity or RegisterSettingEntity or None = (
                self.__connector.get_setting_by_address(device, settings_type, i)
            )

            if setting is None:
                self.__connector.create_setting(device, i, settings_type)

        if settings_size < len(
            self.__connector.get_settings_by_type(device, settings_type)
        ):
            for i in range(
                settings_size,
                len(self.__connector.get_settings_by_type(device, settings_type)),
            ):
                setting: DeviceSettingEntity or RegisterSettingEntity or None = (
                    self.__connector.get_setting_by_address(device, settings_type, i)
                )
    ...
    def __send_data_to_device(self, device: DeviceEntity, data: List[int]) -> None:
        ...
        result: bool = self.__connector.send_packet(
            device.get_address(), data, self.__PACKET_RESPONSE_DELAY
        )
    ...
    def __broadcast_pairing_request_handler(self) -> None:
        # 0 => Packet identifier
        # 1 => Pairing command
    ...
    def __send_set_address_handler(self, device: DeviceEntity) -> None:
        # 0     => Packet identifier
        # 1     => Pairing command
    ...
    def __send_pairing_cmd_handler(
        self, device: DeviceEntity, command: PairingCommands
    ) -> None:
        # 0 => Packet identifier
        # 1 => Pairing command
        # 2 => Packet null terminator
    ...
    def __send_provide_registers_structure_handler(self, device: DeviceEntity) -> None:
        ...
        if registers_type == RegistersTypes.FB_REGISTER_AI:
            ...
            raise InvalidArgumentException(
                "Provided register type: {} is not valid".format(registers_type)
            )
    ...
    def __send_provide_settings_structure_handler(self, device: DeviceEntity) -> None:
        ...
        if setting_type == SettingsTypes.FB_SETTINGS_DEVICE:
            settings_size: int = len(
                self.__connector.get_settings_by_type(
                    device, SettingsTypes(SettingsTypes.FB_SETTINGS_DEVICE)
                )
            )
            ...
            settings_size: int = len(
                self.__connector.get_settings_by_type(
                    device, SettingsTypes(SettingsTypes.FB_SETTINGS_REGISTER)
                )
            )

        else:
            raise InvalidArgumentException(
                "Provided setting type: {} is not valid".format(setting_type)
            )
    ...
    def __device_address_receiver(self, sender_address: int, payload: str) -> None:
        # Get sender supported max packet size
        device_current_address: int = int(payload[2])

        if device_current_address != sender_address:
            log.warn(
                "Received packet with address mismatch: {} vs {}".format(
                    sender_address, device_current_address
                )
            )
        ...
        device: DeviceEntity or None = self.__connector.get_device_by_serial_number(
            serial_number
        )
        ...
        if device is None:
            ...
            device: DeviceEntity = self.__connector.create_device(
                serial_number, pjon.PJON_PACKET_MAX_LENGTH
            )

            log.debug(
                "New device with SN: {} was successfully added to registry with address: {}".format(
                    device.get_serial_number(), device.get_address()
                )
            )
            ...
            if device.get_address() == self.__connector.ADDRESS_NOT_ASSIGNED:
                ...
                device_by_address: DeviceEntity or None = (
                    self.__connector.get_device_by_address(device_current_address)
                )

                # ...and check if is same as device from whom packet was received
                if (
                    device_by_address is not None
                    and device.get_serial_number() != device.get_serial_number()
                ):
                    log.warn(
                        "Received serial number: {} is not unique".format(serial_number)
                    )
        ...
        if device_current_address == self.__connector.ADDRESS_NOT_ASSIGNED:
            ...
            device.set_pairing_cmd(
                PairingCommands(PairingCommands.FB_PAIRING_CMD_SET_ADDRESS)
            )
            ...
            # Define next pairing command
            device.set_pairing_cmd(
                PairingCommands(PairingCommands.FB_PAIRING_CMD_PROVIDE_ABOUT_INFO)
            )
    ...
    def __device_address_accepted_receiver(
        self, sender_address: int, payload: str
    ) -> None:
    ...
    def __device_about_info_receiver(self, sender_address: int, payload: str) -> None:
        # Get device info from registry
        device: DeviceEntity or None = self.__connector.get_device_by_address(
            sender_address
        )
        ...
        if device.has_description_support():
            # Define next pairing command
            device.set_pairing_cmd(
                PairingCommands(PairingCommands.FB_PAIRING_CMD_PROVIDE_DEVICE_MODEL)
            )

        else:
            # Define next pairing command
            device.set_pairing_cmd(
                PairingCommands(PairingCommands.FB_PAIRING_CMD_PROVIDE_REGISTERS_SIZE)
            )
    ...
    def __device_description_receiver(
        self, response: PairingResponses, sender_address: int, payload: str
    ) -> None:
        # Get device info from registry
        device: DeviceEntity or None = self.__connector.get_device_by_address(
            sender_address
        ...
        if response == PairingResponses.FB_PAIRING_RESPONSE_DEVICE_MODEL:
            ...
            log.debug(
                "Received device model: {} for device with address: {}".format(
                    content, device.get_address()
                )
            )
            ...
            # Define next pairing command
            device.set_pairing_cmd(
                PairingCommands(PairingCommands.FB_PAIRING_CMD_PROVIDE_DEVICE_VERSION)
            )

            log.debug(
                "Received device manufacturer: {} for device with address: {}".format(
                    content, device.get_address()
                )
            )
            ...
            log.debug(
                "Received device version: {} for device with address: {}".format(
                    content, device.get_address()
                )
            )
        ...
        # FIRMWARE
            ...
            # Define next pairing command
            device.set_pairing_cmd(
                PairingCommands(PairingCommands.FB_PAIRING_CMD_PROVIDE_FIRMWARE_VERSION)
            )

            log.debug(
                "Received device firmware manufacturer: {} for device with address: {}".format(
                    content, device.get_address()
                )
            )
            ...
            log.debug(
                "Received device firmware version: {} for device with address: {}".format(
                    content, device.get_address()
                )
            )
    ...
    def __registers_size_receiver(self, sender_address: int, payload: str) -> None:
        # Get device info from registry
        device: DeviceEntity or None = self.__connector.get_device_by_address(
            sender_address
        )
        ...
        self.__configure_registers(
            device, int(payload[2]), RegistersTypes(RegistersTypes.FB_REGISTER_DI)
        )

        # DO registers
        self.__configure_registers(
            device, int(payload[3]), RegistersTypes(RegistersTypes.FB_REGISTER_DO)
        )

        # AI registers
        self.__configure_registers(
            device, int(payload[4]), RegistersTypes(RegistersTypes.FB_REGISTER_AI)
        )

        # AO registers
        self.__configure_registers(
            device, int(payload[5]), RegistersTypes(RegistersTypes.FB_REGISTER_AO)
        )

        log.debug(
            "Configured registers: (DI: {}, DO: {}, AI: {}, AO: {}) for device with address: {}".format(
                len(
                    self.__connector.get_registers_by_type(
        ...
        if (
            ...
            if device.has_settings_support():
                ...
                # Define next pairing command
                device.set_pairing_cmd(
                    PairingCommands(PairingCommands.FB_PAIRING_CMD_FINISHED)
                )
    ...
    def __registers_structure_receiver(
        ...
        registers: List[RegisterEntity] = self.__connector.get_registers_by_type(
            device, registers_type
        )
        ...
        if len(registers) > (start_address + registers_length):
            ...
            device.set_reading_register(
                (start_address + registers_length), registers_type
            )
        ...
        # Check if device has AO registers to initialize
            ...
            # Define next pairing command
            device.set_pairing_cmd(
                PairingCommands(PairingCommands.FB_PAIRING_CMD_PROVIDE_SETTINGS_SIZE)
            )
    ...
    def __settings_size_receiver(self, sender_address: int, payload: str) -> None:
        # Get device info from registry
        device: DeviceEntity or None = self.__connector.get_device_by_address(
            sender_address
        )
        ...
        self.__configure_settings(
            device, int(payload[2]), SettingsTypes(SettingsTypes.FB_SETTINGS_DEVICE)
        )

        # Registers settings registers
        self.__configure_settings(
            device, int(payload[3]), SettingsTypes(SettingsTypes.FB_SETTINGS_REGISTER)
        )

        log.debug(
            "Configured settings: (Device: {}, Registers: {}) for device with address: {}".format(
                len(
                    self.__connector.get_settings_by_type(
                        device, SettingsTypes(SettingsTypes.FB_SETTINGS_DEVICE)
                    )
        ...
        if (
            ...
            device.set_reading_setting(
                0, SettingsTypes(SettingsTypes.FB_SETTINGS_DEVICE)
            )
            ...
            # Set setting reading address
            device.set_reading_setting(
                0, SettingsTypes(SettingsTypes.FB_SETTINGS_REGISTER)
            )
    ...
    def __settings_structure_receiver(
        ...
        for i in range(start_address, payload_length):
            if settings_type == SettingsTypes.FB_SETTINGS_DEVICE:
                ...
                setting: DeviceSettingEntity or None = (
                    self.__connector.get_setting_by_address(device, settings_type, i)
                )
                ...
                if setting is not None:
                    if DataTypes.has_value(int(payload[byte_pointer])):
                        ...
                        name: str = Helpers.extract_text_from_payload(
                            payload, byte_pointer + 1
                        )
                ...
                # Find setting in model
                setting: RegisterSettingEntity or None = (
                    self.__connector.get_setting_by_address(device, settings_type, i)
                )

                if setting is not None:
                    if RegistersTypes.has_value(int(payload[byte_pointer + 2])):
                        register_address: int = (int(payload[byte_pointer]) << 8) | int(
                            payload[byte_pointer + 1]
                        )

                        register_type: RegistersTypes = RegistersTypes(
                            int(payload[byte_pointer + 2])
                        )

                        register: RegisterEntity or None = (
                            self.__connector.get_register_by_address(
                        ...
                        if register is not None:
                            if DataTypes.has_value(int(payload[byte_pointer + 3])):
                                data_type: DataTypes = DataTypes(
                                    int(payload[byte_pointer + 3])
                                )

                                name: str = Helpers.extract_text_from_payload(
                                    payload, byte_pointer + 4
                                )

                                setting.set_register(
                                    register.get_address(), register.get_type()
                                )
                                setting.set_data_type(data_type)
                                setting.set_name(name)
                                ...
                                log.warn(
                                    "Received register setting with unknown data type"
                                )
        ...
        if len(settings) > (start_address + registers_counter):
            ...
            device.set_reading_setting(
                (start_address + registers_counter), settings_type
            )
    ...
    def __pairing_finished_receiver(self, sender_address: int, payload: str) -> None:
        # Get device info from registry
        device: DeviceEntity or None = self.__connector.get_device_by_address(
            sender_address
        )
