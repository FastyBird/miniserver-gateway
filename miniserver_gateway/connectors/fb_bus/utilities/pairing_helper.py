...
class PairingHelper:
    ...
    def receive(self, packet: Packets, sender_address: int, payload: str, length: int) -> None:
        ...
        if packet == Packets.FB_PACKET_PAIR_DEVICE:
            ...
            if pairing_response == PairingResponses.FB_PAIRING_RESPONSE_DEVICE_ADDRESS:
                ...
                self.__device_description_receiver(pairing_response, sender_address, payload)
                ...
                self.__registers_structure_receiver(sender_address, payload)
                ...
                self.__settings_structure_receiver(sender_address, payload)
        ...
        # Device for pairing is assigned
            ...
            if pairing_device.get_pairing_cmd() == PairingCommands.FB_PAIRING_CMD_SET_ADDRESS:
                if pairing_device.get_state() != DeviceStates.STATE_CONNECTED:
                    log.debug(
                        "Device is in invalid state, paring could not be finished".format(pairing_device.get_address())
                    )
                ...
                if (
                    ...
                    self.__send_pairing_cmd_handler(pairing_device, pairing_device.get_pairing_cmd())
    ...
    def __configure_registers(self, device: DeviceEntity, registers_size: int, registers_type: RegistersTypes) -> None:
        is_binary: bool = False

        if registers_type == RegistersTypes.FB_REGISTER_DI or registers_type == RegistersTypes.FB_REGISTER_DO:
            is_binary = True
    ...
    def __configure_settings(self, device: DeviceEntity, settings_size: int, settings_type: SettingsTypes) -> None:
    ...
    def __send_data_to_device(self, device: DeviceEntity, data: List[int]) -> None:
        ...
        result: bool = self.__connector.send_packet(device.get_address(), data, self.__PACKET_RESPONSE_DELAY)
    ...
    def __send_pairing_cmd_handler(self, device: DeviceEntity, command: PairingCommands) -> None:
        # 0 => Packet identifier
        # 1 => Pairing command
        output_content: List[int] = [
            Packets(Packets.FB_PACKET_PAIR_DEVICE).value,
    ...
    def __send_provide_registers_structure_handler(self, device: DeviceEntity) -> None:
        ...
        max_readable_registers_count: int = device.get_max_packet_length() - 6
    ...
    def __send_provide_settings_structure_handler(self, device: DeviceEntity) -> None:
        ...
        if setting_type == SettingsTypes.FB_SETTINGS_DEVICE:
            settings_size: int = len(
                self.__connector.get_settings_by_type(device, SettingsTypes(SettingsTypes.FB_SETTINGS_DEVICE))
            )

        elif setting_type == SettingsTypes.FB_SETTINGS_REGISTER:
            settings_size: int = len(
                self.__connector.get_settings_by_type(device, SettingsTypes(SettingsTypes.FB_SETTINGS_REGISTER))
            )
        ...
        max_readable_settings_count: int = device.get_max_packet_length() - 6
    ...
    def __device_address_accepted_receiver(self, sender_address: int, payload: str) -> None:
    ...
    def __device_description_receiver(self, response: PairingResponses, sender_address: int, payload: str) -> None:
        # Get device info from registry
        device: DeviceEntity or None = self.__connector.get_device_by_address(sender_address)
        ...
        if response == PairingResponses.FB_PAIRING_RESPONSE_DEVICE_MODEL:
            ...
            log.debug("Received device model: {} for device with address: {}".format(content, device.get_address()))
            ...
            log.debug(
                "Received device manufacturer: {} for device with address: {}".format(content, device.get_address())
            )
            ...
            log.debug("Received device version: {} for device with address: {}".format(content, device.get_address()))
        ...
        # FIRMWARE
            ...
            log.debug(
                "Received device firmware version: {} for device with address: {}".format(content, device.get_address())
            )
    ...
    def __registers_structure_receiver(self, sender_address: int, payload: str) -> None:
        # Get device info from registry
        device: DeviceEntity or None = self.__connector.get_device_by_address(sender_address)
        ...
        for i in range(start_address, (start_address + registers_length)):
        ...
        # Check if device has AO registers to initialize
            ...
            and len(self.__connector.get_registers_by_type(device, RegistersTypes(RegistersTypes.FB_REGISTER_AO))) > 0
        ):
    ...
    def __settings_size_receiver(self, sender_address: int, payload: str) -> None:
        ...
        if len(self.__connector.get_settings_by_type(device, SettingsTypes(SettingsTypes.FB_SETTINGS_DEVICE))) > 0:
            ...
            # Set reading to default
            device.reset_reading_setting(True)
    ...
    # 5     => High byte of registers length    => 0-255
    # 6     => Low byte of registers length     => 0-255
    # 7-n   => Settings structure               => 0-255

    def __settings_structure_receiver(self, sender_address: int, payload: str) -> None:
        # Get device info from registry
        device: DeviceEntity or None = self.__connector.get_device_by_address(sender_address)
        ...
        for i in range(start_address, (start_address + read_length)):
            if settings_type == SettingsTypes.FB_SETTINGS_DEVICE:
                # Find setting in model
                setting: DeviceSettingEntity or None = self.__connector.get_setting_by_address(device, settings_type, i)
        ...
        # Check if device has registers settings to initialize
            ...
            and len(self.__connector.get_settings_by_type(device, SettingsTypes(SettingsTypes.FB_SETTINGS_REGISTER)))
            > 0
        ):
